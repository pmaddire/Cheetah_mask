...
function alpha_percent_errors = compute_errors_for_all_alpha_D4(alpha_values)
    ...
    for alpha_idx = 1:num_alphas
       ...
       % umap_FG = inv(inv(Sigma_0) + N * inv(FG_sample_cov)) * (inv(Sigma_0) * mu0_FG' + N * inv(FG_sample_cov) * FG_sample_mean');
        ...
        %umap_FG = inv(inv(Sigma_0)+N_FG*inv(FG_sample_cov))*(inv(Sigma_0)*mu0_FG'+N_FG*inv(FG_sample_cov)*FG_sample_mean');
        %umap_BG = inv(inv(Sigma_0)+N_BG*inv(BG_sample_cov))*(inv(Sigma_0)*mu0_BG'+N_BG*inv(BG_sample_cov)*BG_sample_mean');
        
        umap_FG = (Sigma_0*N_FG)*inv(Sigma_0*N_FG + FG_sample_cov)*FG_sample_mean'+FG_sample_cov*inv(Sigma_0*N_FG + FG_sample_cov)*mu0_FG';
        umap_BG = (Sigma_0*N_BG)*inv(Sigma_0*N_BG + BG_sample_cov)*BG_sample_mean'+BG_sample_cov*inv(Sigma_0*N_BG + BG_sample_cov)*mu0_BG';
        ...
        for i = 1:numRows
            for j = 1:numCols
                ...
                for s = 1:64
                    x = Data_array(cnt, s);
                    logPxgivenCheeta = logPxgivenCheeta + likliehood_FG(FG_posterior_mean, FG_sample_cov, s, x);
                    logPxgivenGrass = logPxgivenGrass + likliehood_BG(BG_posterior_mean, BG_sample_cov, s, x);
                end
